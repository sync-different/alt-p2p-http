...
- **First production use** (same day): alt-core's web UI (localhost:8081, the uiv5
  ...
  coordinator (its address lives in ops notes, not here), browsed from a second machine.
  Both peers behind one NAT: the coordinator's LAN-endpoint exchange kicked in and the
  punch went direct over the LAN — no hairpin, no relay. "Worked perfectly" (Alejandro).
